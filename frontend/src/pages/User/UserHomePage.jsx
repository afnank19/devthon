...
export default function UserHomePage() {
  ...
  useEffect(() => {
    ...
    const allCourses = [
      {
        ...
        description: 'Learn the basics of vocal training',
        progress: 50
      },
      {
        id: 2,
      ...
      {
        id: 3,
        title: 'Singing for Performance',
        description: 'Prepare for live performances',
        progress: 100
      }
    ];
    ...
    setAvailableCourses(
      allCourses.filter(
        (course) =>
          !userEnrolledCourses.some((enrolled) => enrolled.id === course.id)
      )
    );
  ...
  useEffect(() => {
    const { role, aTkn } = localStorage;
    console.log(role, aTkn);
  ...
  const joinCourse = (course) => {
    ...
    setAvailableCourses((prev) =>
      prev.filter((available) => available.id !== course.id)
    );
  };
  ...
  useEffect(() => {
    const fetchEnrollments = async () => {
      try {
        ...
        const url = `http://localhost:3000/users/${userId}/enrollments`;

        // Send a POST request passing the userId in the request body
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-gray-700 text-white  transition-all duration-500 ease-in-out">
        ...
        {enrolledCourses.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate__animated animate__fadeIn animate__delay-1s">
            {enrolledCourses.map((course) => (
              <div
                key={course.id}
                className="bg-slate-500 p-8 rounded-lg shadow-xl transform transition-all hover:scale-105 hover:bg-slate-400 ease-in-out duration-300"
              >
                ...
                <Link
                  to={`/course/${course.id}`}
                  className="text-blue-400 hover:text-white mt-4 inline-block"
                >
                  View Course Details
                </Link>
              </div>
      ...
      <section className="mt-12">
        ...
        {availableCourses.length > 0 ? (
          ...
          <p className="text-center text-xl">
            No available courses. Check back later for more!
          </p>
        )}
      </section>
    </div>
